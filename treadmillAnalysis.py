...
class treadmillAnalysis():
	...
	def builddb(self, sessionStr):
		...
		print 'treadmillAnalysis.builddb() writing dbfile:', dbFile, 'sessionStr:', sessionStr
		dbID = open(dbFile, 'w')
	...
	def plottrialparams(self, trialDict):
		...
		'''
		print 'treadmillAnalaysis.plottrialparams()'
		#print 'trialDict:', trialDict
		preDur = long(trialDict['preDur'])
		postDur = long(trialDict['postDur'])
		numEpoch = long(trialDict['numEpoch'])
		epochDur = long(trialDict['epochDur'])
		...
		totalDur = preDur + (numEpoch*epochDur) + postDur
		...
		if useMotor=='motorOn':
			...
			for i in np.arange(numEpoch):
				#motorStart = preDur + (i*pulseDur) + motorDel
				motorStart = preDur + (i*epochDur) + motorDel
