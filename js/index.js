           ...
           //var trialIsRunning = 0;
            ...
            console.log('index.js is creating socket');
            ...
            //socket.on('stoptrial', function(msg) {
        	//	console.log('received stoptrial');
        	//	trialspinner(0);
            //});

            socket.on('serverUpdate', function(msg) {
                	json = $.parseJSON(msg);
					//console.log('serverUpdate:' + json);
                	...
                	$('#frameNumberID').empty().append('Frame #: ' + json.frameNumber);
                	$('#elapsedSecondsID').empty().append('Elapsed (sec): ' + json.elapsedSeconds);
                	...
                	//$('#currentDurID').empty().append('Duration (ms): ' + json.dur);
                 	...
                 	$('#armVideoCheckboxID').prop('checked', json.cameraArmed);                	
                 	$('#armStartTriggerCheckboxID').prop('checked', json.armStartTrigger);                	
               	...
               	if (json.trialRunning=='1') {
					...
					//console.log('epochNumber:' + json.epochNumber);
					//console.log('serverUpdate:' + json.currenttime + ' ' + json.trialRunning);
            });
			
            socket.on('serialdata', function(msg) {
                //console.log('index.js received serialdata:' + msg.data);
			...
			$(".checkboxCallback").change(function() {
				//alert(this.id);
				if (this.id == "armVideoCheckboxID") {
					if (this.checked) {
						console.log('armVideoCheckboxID checked');
				  		socket.emit('armvideo', {data: '1'});
					} else {
						console.log('armVideoCheckboxID unchecked');
				  		socket.emit('armvideo', {data: '0'});
					}
				}
				if (this.id == "armStartTriggerCheckboxID") {
					console.log('armStartTriggerCheckboxID:' + this.checked);
					socket.emit('armtrigger', {data: this.checked});
				}
			});
        ...
        chart: {
            ...
            events: {
                load: function() {
                    ...
                    socket.on('trialupdate', function (msg) {
